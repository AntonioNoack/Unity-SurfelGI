Shader "Custom/SurfelAccuShader" {
    // accumulates the emissions of all surfels
    // weights them by surface properties, like normal, and smoothness
    Properties { }
    SubShader {
        ...
        Pass {
            ...
            v2f vert (uint vertexId: SV_VertexID, uint instanceId: SV_InstanceID) {
                ...
                UNITY_INITIALIZE_OUTPUT(v2f, o);
                    ...
                    o.surfelId = instanceId;
                    Surfel surfel = (Surfel) 0;
                    if(instanceId < _SurfelCount) {
                        surfel = _Surfels[instanceId];
                        localPos = quatRot(localPos * float3(1.0,1.0,1.0), surfel.rotation) * surfel.position.w + surfel.position.xyz;
                        if(surfel.color.w < 0.0001) localPos = 0; // invalid surfel / surfel without known color
                    }
                #endif
